...
function renderArticleContent(rawText: string) {
  ...
  const flushParagraph = (keyPrefix: number | string) => {
    if (paragraphBuffer.length > 0) {
      ...
      if (text) {
        elements.push(
          <p key={`p-${keyPrefix}`} className="text-gray-700 leading-relaxed text-base md:text-lg mb-6 whitespace-pre-line">
            {text}
          </p>
        );
      }
  ...
  lines.forEach((line, index) => {
    ...
    if (imgMatch) {
      ...
      elements.push(
        <div key={`img-${index}`} className="my-8 rounded-2xl overflow-hidden shadow-md border border-gray-100 bg-gray-50 flex flex-col items-center">
          <img
            src={src}
            alt={alt}
            className="w-full max-h-[750px] object-contain mx-auto rounded-xl"
            loading="lazy"
          />
          {alt && alt !== 'Imagen de la noticia' && !alt.startsWith('http') && (
            <p className="text-center text-xs text-gray-500 py-2.5 bg-gray-50/90 border-t border-gray-100 w-full">
              {alt}
            </p>
          )}
        </div>
